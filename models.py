...
def modelo(model, lat='full', lon='full'):
	success, out = db_rendler._pull_model_data(model.type, model.name, model.member, lat, lon)
	if success == True:
		f_id = []
		f_path = []
		for val in out:
...
def data(_start, _end, lat='full', lon='full'):
	success, out = db_rendler._pull_date_data(date, lat, lon)
	if success == True:
		f_id = []
		f_path = []
		for val in out:
...
def descricao(element='all'):
	...
	elif element == "cenarios":
		success, out.cenario 	= db_rendler._get_cenarios()
	elif element == "all":
